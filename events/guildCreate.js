    ...
    async run (guild) {
        ...
        if(this.client.config.proMode){
            if((!this.client.config.proUsers.includes(guild.ownerID) || this.guilds.filter((g) => g.ownerID === guild.ownerID) > 1) && guild.ownerID !== this.client.config.owner.id){
                this.client.logger.log(guild.ownerID+" tried to invite Cryptic to their server.");
                return guild.leave();
            }
        }
        ...
        let text = "I joined **"+guild.name+"**, with **"+guild.members.filter((m) => !m.user.bot).size+"** members (and "+guild.members.filter((m) => m.user.bot).size+" bots)";
