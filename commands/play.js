...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!args.length) {
      ...
      return message.channel.send("WRONG SYNTAX : Type `c!play <URL> or text`");
    }

    const { channel } = message.member.voice;
    if (!channel) {
      //IF AUTHOR IS NOT IN VOICE CHANNEL
      return message.channel.send("I can't play that song unless you are in a Voice Channel!");
    }
    ...
    if (!videoPattern.test(args[0]) && playlistPattern.test(args[0])) {
      return message.channel.send("Links cannot be processed yet.");
    }
